...
window._getFirstRowColumnIndexForMajorDiagonalOn = function(rowIndex, colIndex) {
...
//       2    [0,0,1,0]
//       3    [0,1,0,0]
window._getFirstRowColumnIndexForMinorDiagonalOn = function(rowIndex, colIndex) {
  return colIndex + rowIndex;
},

window.hasMajorDiagonalConflictAt= function(colI, n, arr) {
  let flag = false;
  for(let row=0; row < n; row++){
    for(let col = colI; col < n; col++){
      if(colI === (col-row)){
        if(arr[row][col] === 1){
          if(flag === false){
            flag = true;
...
window.hasAnyMajorDiagonalConflicts = function(n, arr) {
  for(let row=0; row<n; row++){
    for(let col=0; col<n; col++){
      let majorDiagonalColumnIndexAtFirstRow = this._getFirstRowColumnIndexForMajorDiagonalOn(row,col);
      if(this.hasMajorDiagonalConflictAt(majorDiagonalColumnIndexAtFirstRow,n,arr) === true) return true;
    }
  }
  return false; // fixme
}
...
window.hasMinorDiagonalConflictAt = function(minorDiagonalColumnIndexAtFirstRow, n, arr) {
  let flag = false;
  for(let row = 0; row < n; row++){
...
window.hasAnyMinorDiagonalConflicts = function(n, arr) {
  for(let row=0; row<n; row++){
    for(let col=0; col<n; col++){
      let minorDiagonalColumnIndexAtFirstRow = this._getFirstRowColumnIndexForMinorDiagonalOn(row,col);
      if(this.hasMinorDiagonalConflictAt(minorDiagonalColumnIndexAtFirstRow,n , arr) === true) return true;
    }
  }
  return false; // fixme
}
